...
@interface DLNAUpnpServer () <GCDAsyncUdpSocketDelegate>
@property (nonatomic, strong) GCDAsyncUdpSocket                         *udpSocket;
@property (nonatomic, strong) NSMutableDictionary<NSString *, DLNADevice *> *deviceDic; // key: usn(uuid) string,  value: device
...
- (void)startAndSearch:(BOOL)isSearch
{
    ...
    {
        DLNALog(@"===============>> Error bindToPort: %@", [error description]);
        [_udpSocket bindToPort:0 error:nil];
    }
    
    if (![_udpSocket beginReceiving:&error])
    {
        DLNALog(@"===============>> Error beginReceiving: %@", [error description]);
    }
    
    if (![_udpSocket joinMulticastGroup:UDP_SERVER_HOST error:&error])
    {
        DLNALog(@"===============>> Error joinMulticastGroup: %@", [error description]);
    }
...
{
    NSMutableArray<DLNADevice *> *array = [[NSMutableArray alloc] init];
    if (self.deviceDic)
    {
...
- (DLNADevice *)parserDeviceLocation:(NSString *)location withUSN:(NSString *)usn
{
    dispatch_semaphore_t seamphore = dispatch_semaphore_create(0);
    __block DLNADevice *device = nil;
            ...
            if ([httpResponse statusCode] == 200) {
                ...
                for (GDataXMLElement *serviceElement in serviceElementArray) {
                    ...
                    if ([serviceId containsString:MEDIA_CONTROL_SERVICE_ID] || [serviceType containsString:SERVICE_TYPE_AVTRANSPORT]) {
                        DLNAMediaControlService *service = [[DLNAMediaControlService alloc] init];
                    ...
                    if ([serviceId containsString:RENDERING_CONTROL_SERVICE_ID] || [serviceType containsString:SERVICE_TYPE_RENDERING_CONTROL]) {
                        DLNARenderingControlService *service = [[DLNARenderingControlService alloc] init];
                        service.type = [[[serviceElement elementsForName:@"serviceType"] objectAtIndex:0] stringValue];
                        NSString *format;
                        NSString *controlUrl = [[[serviceElement elementsForName:@"controlURL"] objectAtIndex:0] stringValue];
                        if (![controlUrl hasPrefix:@"/"]) {
...
- (void)addDevice:(DLNADevice *)device forUSN:(NSString *)usn
{
    if (!device)
    {
        return;
    ...
    DLNALog(@"===============>> add device: \"%@\", uuid: \"%@\"", device.name, usn);
...
{
    DLNALog(@"===============>> remove device uuid: \"%@\"", usn);
...
//    [GCDAsyncUdpSocket getHost:&host port:&port fromAddress:address];
    ...
    else if ([str hasPrefix:@"HTTP/1.1 200 OK"])
    {
        ...
        NSString *location = [self headerValueForKey:@"Location:" inData:str];
        NSString *usn = [self headerValueForKey:@"USN:" inData:str];
        DLNALog(@"===============>> search response data %@", str);
...
{
    DLNALog(@"===============>> system close udp socket");
