...
@interface MainViewController ()<DLNADeviceChangeDelegate>
...
- (void)viewDidLoad {
    ...
    [[DLNAFileServer shareServer] start];
}
...
- (void)selectDevice {
    ...
    deviceVC.selectedDevice = ^(int index) {
        ...
        DLNADevice * device = [[[DLNAUpnpServer shareServer] getDeviceList] objectAtIndex:index];
...
- (void)selectPhoto {
    ...
    photoViewController.address = ^(NSString *address) {
        self.targetAddressLabel.text = address;
    };
